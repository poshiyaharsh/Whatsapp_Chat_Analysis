...
def emoji_helper(selected_user,df):
    ...
    emojis = []
    for message in df['message']:
        # For modern versions of the `emoji` package, use `emoji.is_emoji`
...
def sentiment_overview(selected_user, df):
...
def message_length_stats(selected_user, df):
    ...
    avg_words = round(df['word_count'].mean(), 2) if not df.empty else 0

    return df['word_count'], avg_words
